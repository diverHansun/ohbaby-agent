...
<p align="center">
  <img src="assets/images/dashboard.png" alt="ohbaby terminal UI" width="760">
</p>

---

**ohbaby** is an AI coding agent that lives in your terminal. It is provider-agnostic,
extensible through [MCP](https://modelcontextprotocol.io) servers and drop-in skills,
and ships with a fast [Ink](https://github.com/vadimdemedes/ink)-based TUI. Bring your
own API key from any OpenAI-compatible or Anthropic provider and start coding.
...
- **🛠️ Skills** — extend ohbaby with reusable skills that show up as slash commands.
...
- **💬 Interactive TUI** — slash commands, session history, model switching, and live
  streaming output.
...
**2. Connect a model.** In the TUI, type `/connect` to open the provider setup panel,
fill in your provider, base URL, API key, and model name, then save.

<p align="center">
  <img src="assets/images/connect-providers.png" alt="ohbaby /connect provider setup" width="760">
</p>
...
**3. Start coding.** That's it — describe what you want and ohbaby gets to work.
...
Skills are discovered from ohbaby-compatible skill directories and exposed as slash
...
- **`ohbaby-cli`** — the `ohbaby` binary, startup commands, stdout renderer, and TUI.
- **`ohbaby-agent`** — the backend runtime: adapters, tools, sessions, policy, MCP, skills.
- **`ohbaby-sdk`** — stable TypeScript contracts shared between runtime and frontends.
