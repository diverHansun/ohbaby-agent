...
export class ShellCommandPolicyError extends Error {
  constructor(message: string) {
    super(message);
    this.name = "ShellCommandPolicyError";
  }
}
...
function reject(message: string): never {
  throw new ShellCommandPolicyError(message);
}
...
function npmCommandClass(args: readonly string[]): ShellCommandClass {
  const subcommand = args.find((arg) => !arg.startsWith("-"))?.toLowerCase();
  return subcommand === "install" || subcommand === "i"
    ? "mutating"
    : "mutating";
}
...
function detailCommandClass(detail: CommandDetail): ShellCommandClass {
  ...
  if (root === "npm" || root === "pnpm" || root === "yarn") {
    return npmCommandClass(args);
  }
  if (SHELL_EXEC_COMMANDS.has(root) || root === "xargs") {
    return "mutating";
  }
