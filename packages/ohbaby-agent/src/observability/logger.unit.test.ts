...
describe("diagnostic normalization", () => {
  ...
  it("redacts auth-header and bearer credential shapes in retained paths", () => {
    for (const credentialPath of [
      ...
      "logs/api.key=DYNAMIC_API_KEY",
      "logs/private.key=DYNAMIC_PRIVATE_KEY",
      "logs/authorization.header=Basic DYNAMIC_AUTH",
      "logs/secret[key]=DYNAMIC_SECRET",
      `logs/secret${"x".repeat(129)}=DYNAMIC_PADDED_SECRET`,
    ]) {
