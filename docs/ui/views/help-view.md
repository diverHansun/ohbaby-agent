...
数据驱动渲染。命令列表从 `cli/commands` 模块动态获取，按 category 分组展示。

**数据来源**：调用 `cli/commands` 模块提供的命令注册查询接口，获取所有已注册命令及其分类和描述信息。使用 useMemo 缓存，仅在组件挂载时计算一次。
...
| tools | /tools, /mcp |
| system | /help, /status, /init, /exit, /approval-mode, /agents, /memory |
...
当 commands 模块新增或移除命令时，HelpView 的右栏自动反映变化，无需修改 View 代码。新增命令只需在 commands 模块中注册并声明 category，HelpView 即可展示。
...
| CommandsColumn | HelpView 内部组件 | cli/commands 模块（动态获取） |

ShortcutsColumn 和 CommandsColumn 是 HelpView 的内部子组件，不对外导出，不在其他视图中复用。

## 六、Context 依赖

HelpView 不读取任何 React Context。

- 命令数据通过直接调用 `cli/commands` 模块函数获取，不通过 Context
- 快捷键数据为硬编码静态内容
...
3. **不展示命令详细用法**：仅展示命令名和一句话描述。详细用法通过 `/help [command]` 命令查看
4. **不硬编码命令列表**：右栏命令全部通过 cli/commands 模块动态获取
