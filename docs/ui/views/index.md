...
| 打开帮助视图 | command result handler 收到 `action.kind='show-help'` | `navigateTo('help')` |
...
| 文档 | 说明 |
|------|------|
| [index.md](./index.md) | 视图层概述、Router 路由机制、设计原则 |
| [home-view.md](./home-view.md) | HomeView：品牌标识 + 新手引导，零依赖 |
| [chat-view.md](./chat-view.md) | ChatView：消息列表渲染，TuiStore messages selector |
| [help-view.md](./help-view.md) | HelpView：快捷键（静态）+ 命令目录（数据驱动） |
| [use-case.md](./use-case.md) | 7 个关键用例流程（F1-F7），责任边界与失败点 |
| [non-functional.md](./non-functional.md) | 工程约束：渲染频率隔离、Provider 边界、Router 纯映射、键盘事件禁入 |
| [test.md](./test.md) | 测试策略：混合验证，P0/P1 场景分层，vitest + ink-testing-library |
...
- [x] 新增视图的扩展路径清晰
- [x] 设计原则可追溯到 goals-duty.md（D2、G5）
- [x] non-functional.md 已完成（5 个运行约束：ChatView 渲染隔离、HomeView Provider 边界、Router 纯映射、键盘禁入、HelpView 溢出委托）
- [x] use-case.md 已完成（7 个流程 F1-F7，含 A' 机制说明）
- [x] test.md 已完成（混合验证策略，P0/P1 分层，6 个关键场景）
