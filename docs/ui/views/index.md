...
| ChatView | [chat-view.md](./chat-view.md) | 对话交互主界面 | SessionContext |
| HelpView | [help-view.md](./help-view.md) | 快捷键与命令帮助 | 无（命令数据来自 cli/commands 模块） |
...
| 执行 `/help` 命令 | useInput hook | `navigateTo('help')` |
...
- ChatView：仅依赖 SessionContext（消息数据）
- HelpView：零 Context 依赖，命令数据通过函数调用获取
