...
这是参考 gemini-cli 的 `semantic-colors.ts` 设计。用极小的代码量（6 个 getter）换取未来的主题切换能力。
...
iris-code 借鉴了 gemini-cli 的分层思路和 getter 代理模式，但去掉了所有 MVP 不需要的复杂度。
