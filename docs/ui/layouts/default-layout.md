...
| 条件 | 显示内容 |
|------|---------|
| `isActive = false` | 不渲染（隐藏） |
| `isActive = true, activeToolName = null` | `[spinner] [sword] Thinking...` |
| `isActive = true, activeToolName = "read_file"` | `[spinner] [sword] Executing: read_file` |

### 4.3 状态驱动

加载状态由 Bus 事件驱动，useStream hook 负责监听并更新 AppStateContext：

- `Lifecycle.Event.Started` -- 设置 `isActive = true`
- `ToolScheduler.Event.ExecutionStarted` -- 设置 `activeToolName = toolName`
- `ToolScheduler.Event.ExecutionCompleted` -- 清除 `activeToolName = null`
- `Lifecycle.Event.Completed` -- 设置 `isActive = false, activeToolName = null`

注意：LLM 流式输出期间不产生 Bus 事件，此时加载指示器自然显示"Thinking..."（`isActive` 仍为 true，`activeToolName` 为 null）。
...
| Context | 读取字段 | 用途 |
|---------|---------|------|
| AppStateContext | `loading.isActive`, `loading.activeToolName` | 条件渲染加载指示器 |
