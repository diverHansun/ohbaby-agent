...
为 ohbaby-code 各模块提供统一的底层基础设施，包括日志系统、错误处理基类、生命周期清理机制，确保全局行为一致。
...
当前 ohbaby-code 使用显式参数传递上下文，无需隐式上下文管理。
...
- 可以用一句话说明该模块的存在意义：utils 模块为 ohbaby-code 提供统一的底层工具函数和基础设施
