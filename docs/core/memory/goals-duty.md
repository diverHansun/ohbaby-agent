...
**一句话说明**：memory 模块负责加载、管理和持久化全局与项目级的 OHBABY.md 记忆文件，为 AI 提供长期记忆能力。
...
向记忆文件追加新的条目：
- 支持添加到全局或项目级
- 使用 `## Ohbaby Added Memories` 作为分隔 Header
- 自动添加时间戳和内容
- 不存在时自动创建文件
...
列出所有 AI 添加的记忆条目：
- 解析 `## Ohbaby Added Memories` 下方的列表
- 返回索引、时间戳和内容
- 供 AI 查询和选择修改/删除目标
...
当前版本不支持在 OHBABY.md 中使用 `@import` 引入其他文件（YAGNI）。
...
1. OHBABY.md 文件大小合理（< 1MB），读取性能可接受
2. 同一时刻只有一个进程修改 OHBABY.md（并发冲突概率极低）
3. 用户理解并遵守 `## Ohbaby Added Memories` Header 的约定
4. 文件系统的写入操作是原子性的
...
| 模块 | 代码位置 | 关系 | 调用接口 | 说明 |
|------|----------|------|----------|------|
| Project | `src/project/` | 依赖 | `Project.fromDirectory()` | 获取项目根路径定位 OHBABY.md |
