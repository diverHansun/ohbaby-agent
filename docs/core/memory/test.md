...
describe('Memory.load', () => {
  ...
  it('只有全局记忆时应正确返回', async () => {
    mockFs({
      '~/.ohbaby/OHBABY.md': 'Global content'
      // 项目记忆不存在
    })
    mockProject({ rootPath: '/project' })
...
describe('Memory.add', () => {
  it('应添加记忆到全局文件', async () => {
    ...
    const written = getWrittenContent('~/.ohbaby/OHBABY.md')
...
更多非列表内容
  ...
  it('Header 不存在时应返回空数组', async () => {
    mockFs({
      '~/.ohbaby/OHBABY.md': '用户手写内容，没有 Header'
    })
...
- 2026-01-01 20:10:00: Old text 3
  ...
  it('索引越界时应抛出异常', async () => {
    mockFs({
      '~/.ohbaby/OHBABY.md': '## Ohbaby Added Memories\n- 2026-01-01 20:00:00: Only one'
    })
...
describe('Memory - 错误处理', () => {
  it('文件不可读时应返回空字符串并记录警告', async () => {
    mockFs({
      '~/.ohbaby/OHBABY.md': { readable: false }
    })
...
describe('Integration: Real Filesystem', () => {
  ...
  it('应正确读写真实 OHBABY.md 文件', async () => {
    // 创建全局配置目录
    const globalDir = path.join(tempDir, '.ohbaby')
