...
memory 模块位于 iris-code 的核心层，作为长期记忆管理的中心：
...
| 文件系统 | Node.js fs | 输出 | 直接读写 IRIS.md 文件 |
...
2. [lifecycle -> Memory] 调用 Memory.load(directory)
   ...
   │    │   └── ~/.config/iris-code/IRIS.md (Linux/macOS)
   │    │   └── %APPDATA%/iris-code/IRIS.md (Windows)
   │    ├── 读取文件（不存在返回 ''）
   │    └── globalContent = 文件内容
   │
   ├──> 2.3 [Memory] 加载项目记忆
...
2. 读取全局 IRIS.md（`~/.config/iris-code/IRIS.md`）
3. 从 `directory` 向上查找项目 IRIS.md，找到第一个即停止
...
1. 确定文件路径（全局或项目）
2. 读取当前文件内容
3. 检查是否存在 `## Iris Added Memories` Header
...
4. 只修改 `## Iris Added Memories` 下方的内容
...
**行为说明**：
1. 读取文件内容
2. 定位 `## Iris Added Memories` Header
3. 解析 Header 下方的列表项
4. 只解析以 `-` 开头的行
5. 返回索引化的条目数组
...
| 数据 | 创建者 | 说明 |
|------|--------|------|
| IRIS.md 文件 | Memory 模块（首次 add） | 自动创建目录和文件 |
| 用户手写区域 | 用户 | 手动编辑 |
