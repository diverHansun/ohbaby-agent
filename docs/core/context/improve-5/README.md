...
> 日期：2026-08-21
> 来源：从 [improve-4](../improve-4/README.md) 剥离
>
...
Ohbaby 对外只需要面对 `anthropic` 与 `openai-compatible` 两种 client 请求接口形状，但 cache 匹配、TTL、cache read/write usage、计费与可诊断能力由具体上游服务端决定。该议题同时涉及 interface provider、LLM client、context 占用和未来 cost projection，不能塞进 improve-4 的 tool schema 修复里，也不能先用几个可选字段固定未来模型。
...
- 本批与 improve-4 独立；improve-4 不新增 cache 字段、不启用 cache、不做命中率/成本统计或预测。
