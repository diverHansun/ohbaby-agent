...
> 实施状态：本文保留实施前的问题分析语境。当前分支已完成 P0：per-step context 准备、overflow recovery、tool raw metadata 持久化与 serializer 中央白名单投影。P1/P2/P3 项仍作为后续规划。
...
| **中期** | 会话级压缩、checkpoint、运行时状态 | `ContextManager`（prune → compress → assemble 三阶段流水线），session-based `Lifecycle.run(...)` 管理运行时状态 | 骨架正确，改善见 improve-1 / improve-2 P0 |
...
实施前基线：旧 session lifecycle 仅在 turn 开始时调用一次 `prepareTurn`，之后的 tool-call 循环中 `conversationMessages` 持续增长而不再压缩。如果单轮内 tool 调用密集，可能溢出。当前分支已通过 session-based `Lifecycle.run(...)` 在每个 model step 前重新调用 `prepareTurn`。
...
实施前基线：无此机制，溢出错误直接上报。当前分支已补充 overflow 识别、强制 `prepareTurn({ force: true })` 和单次重试。
...
| Per-step 压缩 | `prepareTurn` 已有完整压缩逻辑 | 已完成：session-based `Lifecycle.run(...)` 在每个 step 前重调 `prepareTurn` |
| 动态 completion budget | `tokenCounter.getBudget()` 已提供 budget | 低：在 `llm-client` 层消费 budget |
| 溢出自动恢复 | 已完成 P0 | 中：已在 session-based lifecycle 中捕获并触发压缩重试 |
...
### PC-17：Per-step 压缩缺失，长 tool 链可能溢出（已完成 P0）

**严重度**：高

**实施前证据**：旧 `Lifecycle.runSession` 仅在 turn 开始时调用一次 `contextManager.prepareTurn()`。之后 `conversationMessages` 数组持续追加 tool 调用消息但不再压缩。

**当前状态**：已由 session-based `Lifecycle.run(params: LifecycleSessionParams)` 替代旧 `runSession`，每个 model step 前都会调用 `prepareTurn` 并发出 `context:prepared`。

**描述**：

```
// 实施前旧 session path 的压缩触发点（仅一次）：
...
### PC-18：上下文溢出无自动恢复（已完成 P0）

**严重度**：高

**实施前证据**：`Lifecycle.runModelStep` 调用 `streamChatCompletion` 时，如果 LLM 返回 context overflow 错误，错误直接传播到上层。

**当前状态**：session-based `Lifecycle.run(...)` 已识别 context overflow，触发强制压缩并重试当前 step 一次；重试仍失败时返回可读失败原因，runtime 保留该原因。
...
- 实施前 ohbaby-agent 缺少这套"检测 → 压缩 → 重试"的自动恢复链路，用户面临原始 API 错误。
...
### PC-25：Tool metadata 缺少中央白名单投影（已完成 P0）

**严重度**：高

**实施前证据**：`core/context/serializer.ts` 的旧 `toolResultContent(part)` 只读取 `part.state.output` 或 `part.state.error`；成功工具结果的 raw metadata 没有进入 `ToolState.completed`，也缺少统一规则决定哪些 metadata 可进入模型上下文。

**当前状态**：`ToolState.completed/error/aborted` 已持久化 raw metadata；`serializeForLlm` 通过 `formatToolResultContentForModel` 做中央白名单投影。

**描述**：

- 实施前旧 `Lifecycle.runSession` 在同一步内能通过内存里的 `ToolCallResult.metadata` 传给下一次 LLM 请求；但 per-step prepare 会从 message store 重建 provider messages，这条内存捷径会消失。
...
实施前 context 的组装 → LLM 输入是硬编码的线性流程。当前 P0 已补上 per-step 检查点和 tool metadata 中央投影；仍缺：
...
消息不记录来源（origin）、文件操作不跨压缩累积、摘要不提升为一等公民。这些是"数据管道中的筛子"——关键信息在流转过程中被丢弃。tool metadata 持久化与模型可见白名单投影已在 P0 完成。
...
| 优先级 | 编号 | 问题 | 影响范围 | 复现难度 | 状态 |
|--------|------|------|----------|---------|------|
| **P0** | PC-17 | Per-step 压缩缺失（实施前） | 长 tool 链场景直接溢出 | 中 | 已完成 |
| **P0** | PC-18 | 溢出无自动恢复（实施前） | 生产可用性 | 中 | 已完成 |
| **P0** | PC-25 | Tool metadata 白名单投影缺失（实施前） | per-step prepare 后执行事实丢失 | 低 | 已完成 |
| **P1** | PC-14 | 事件溯源 | 崩溃恢复、跨进程迁移 | 中 | 后续 |
| **P1** | PC-15 | Origin 追踪 | 调试、精准压缩 | 低 | 后续 |
| **P2** | PC-21 | 文件操作跨压缩累积 | token 经济 | 低 | 后续 |
| **P2** | PC-16 | 注入系统 | 可扩展性 | 中 | 后续 |
| **P2** | PC-19 | 动态 completion budget | API 层防护 | 低 | 后续 |
| **P2** | PC-20 | 后台异步通知 | 多 Agent 效率 | 中 | 后续 |
| **P3** | PC-22 | 摘要跨会话复用 | 知识积累 | 高 | 后续 |
| **P3** | PC-23 | Hooks 系统 | 可扩展性 | 中 | 后续 |
| **P3** | PC-24 | Undo safety | 未来并发安全 | 低 | 后续 |
...
session-based `Lifecycle.run(...)` 在 tool 循环内动态检测是否需要再次压缩，需要时调用 `prepareTurn` 重新组装 context。溢出错误自动触发压缩 + 重试。
