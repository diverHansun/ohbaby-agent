...
| `packages/ohbaby-agent/src/core/context/serializer.ts` | provider message 投影硬编码为 system + memory + history | P2 注入系统入口 |
| `packages/ohbaby-agent/src/core/message/types.ts` | `PartMetadata` 已有 `[key: string]: unknown` | origin 字段优先落在 metadata/info 扩展，不破坏 schema |
...
- 明确首批开发只做 P0：per-step context 准备与 overflow recovery。

### Phase 1：Per-step context 准备的最小正确版本
...
3. 后续 step 若 `prepareTurn` 产生 `compaction`，继续发出可被 UI 投影的 context notice；是否再次发 `turn:start` 由 lifecycle 文档确定。
4. 用 `PreparedTurn.messages` 替换 `conversationMessages`，确保压缩后的 provider messages 来自持久化消息投影。
5. 保留现有内存追加 assistant tool-call/tool-result 的协议测试，避免 OpenAI tool protocol 回归。

**为什么不先做复杂压力探针**：

- 当前 P0 风险是溢出，不是性能。
- `prepareTurn` 已是完整契约，重复调用比新增半成品 `estimateMessagesUsage` 更少抽象。
- 如果性能变成真实问题，再在 Phase 3 增加 `ContextManager.estimateMessagesUsage(...)` 之类窄 API。

### Phase 2：Overflow recovery
...
| 2 | Phase 1 characterization tests |
| 3 | Phase 1 per-step prepare 最小实现 |
| 4 | Phase 2 overflow recovery |
| 5 | Phase 3 dynamic budget |
| 6 | Phase 4/5 可观测性与扩展点（按风险继续拆） |
