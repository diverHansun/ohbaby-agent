...
1. 先读 `problem-analysis.md`，理解：三层记忆架构的正确性确认 → ohbaby-agent 当前优势（S1–S8）→ kimi-code/pi 对比 → 11 个关键问题 → 优先级矩阵 → 优化目标
...
| `core/message` | `MessageWithParts.info` 或 `PartMetadata` 增加 origin/来源信息 | 是（向后兼容字段新增） |
