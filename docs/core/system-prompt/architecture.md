...
- `tools.ts`: 渲染工具片段（tool snippets），并由 assembler 预先过滤可疑内容。
...
```text
base -> primary task -> agent addon -> subagent roles -> tools -> environment -> custom
```
...
tools 层是条件层。只有 `toolSnippets` 产生内容时才会出现在数组中。subagent roles 层也是条件层，只有存在可用 role 时才出现。
