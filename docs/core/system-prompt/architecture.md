...
```
iris-code/src/core/system-prompt/
|-- index.ts                 # 公共 API 导出
...
| - 基础身份描述（你是 iris-code，一个 AI 编程助手...）           |
| - 核心能力说明                                                 |
| - 基础行为准则                                                 |
| - 工具使用指南                                                 |
| - 输出格式规范                                                 |
...
| Custom Instructions | 加载 IRIS.md | 不加载 |
...
**决策**：自定义指令（IRIS.md）在组装时加载，而非启动时缓存。
