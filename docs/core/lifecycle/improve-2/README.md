...
因此 lifecycle improve-2 不再规划"primary 切到 runAgent"。本轮真正要解决的是：**session-run 的长 tool 链韧性、溢出恢复、tool metadata 持久化，以及 `run()` / `runSession()` 双循环债务**。
...
| `ToolPart.state` | completed/error 状态未持久化工具 raw metadata | P0：per-step prepare 后模型会丢失 `mtimeMs`、`exitCode`、MCP `structuredContent` 等执行事实 |
...
在 P0 行为稳定后，删除旧的 message-run 入口：`Lifecycle.run(LifecycleRunParams.messages)` 不再作为生产接口存在。新的唯一 lifecycle 入口是 session-based `Lifecycle.run(params: LifecycleSessionParams, config?: LifecycleConfig)`；迁移期间可以短暂保留 `runSession` 作为重命名过渡，但最终调用方应只依赖新的 `run`。

### G5：工具 raw metadata 持久化

`runSession` 不能再依赖当前 step 内存中的 `ToolCallResult.metadata` 作为下一步模型输入来源。工具执行完成后，raw metadata 必须落入 message store 的 tool part 状态；后续 step 再由 context serializer 做白名单投影。这样 `read -> edit/write` 的 `mtimeMs`、`bash.exitCode`、MCP `structuredContent` 等关键事实不会在 per-step prepare 后丢失。
...
4. 实现 `runSession` 每 step 前重新准备 context 的最小正确版本，并发出 `context:prepared`。
