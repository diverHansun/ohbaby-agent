...
`Tool.source` 只描述工具来源：`builtin`、`module`、`skill`、`mcp`。`skill` 与 `category: "skill"` 不再借用 `module` source。

需要用户再次确认的工具通过 `Tool.requireExplicitApproval === true` 表达。ToolScheduler 在普通 permission state 允许后仍会调用 `Permission.ask()`，传入：
...
MCP adapter 负责把 `trust: false` 映射为 `requireExplicitApproval: true`。ToolScheduler 不再通过 `source === "mcp"` 或 MCP 私有 `isTrusted` 判断是否需要额外确认。
