...
> 状态：实现、文档对齐与本地验收完成；两路独立审查已判定可交付，等待最终修订点复核；未 merge、未 push。
>
...
- logger 使用具名字面量约束的 event definition 与受限字段 encoder，不开放任意 string record；模块级复用是 SHOULD，不为首版自研 AST lint；
...
- 实施阶段按阶段 commit，不在该阶段 merge 或 push。
