...
> 状态：**后端与 Web/TUI Phase E 均已实施；全量 unit/integration、真实多进程 daemon、production build、浏览器可视 E2E 与三路独立复审通过。**
>
...
| [03-reference-projects.md](./03-reference-projects.md) | Codex、Kimi Code、OpenCode 与 ohbaby durable subagent queue 的源码证据、可观察模式、适用前提与 adopt/adapt/reject |
...
- interactive client 为每次提交生成 `clientRequestId`；HTTP `POST /v1/prompts` 与 JSON-RPC `submitPromptAccepted` 将它与 `promptId + userMessageId + sessionId + status` 一并原样返回，并写入 snapshot/event，作为 acceptance 幂等键；只有当前页面 matching receipt 可清空 live draft，SSE/snapshot/replay 只更新投影、绝不直接修改 composer；旧 `submitPrompt` 保持 submit-and-wait，避免默认 TUI/非交互 CLI 提前结束。
...
- Phase E Web 在 composer 上方显示自适应高度的 Queue 区，超过 5 条折叠为 "+N more"；编辑用"弹回 composer" + 60 秒不活动超时的可续租 edit lease，不用 inline pencil edit；cancel 即时生效、无确认、无 undo。
- Phase E TUI 在 composer 上方内联显示队列内容（`↳` 前缀，自适应高度）；`Alt+Up` 弹回最后一条并取得 edit lease，进入明确的 queued-edit 模式，`Ctrl+D` 仅在该模式下取消当前 prompt，Esc 放弃编辑并释放 lease；放弃 `/queue` slash 命令和独立管理面板；默认 TUI 仍保持 in-process。
...
- 固定高度 + 隐藏滚动条 viewport 展示队列（采用自适应高度 + 折叠）。
- inline pencil edit 和只靠 `editing_since` 时间戳的伪锁（采用弹回 composer + 带 owner/token/expiry/renew 的 edit lease）。
- `/queue` slash 命令或独立队列管理面板（采用 TUI 内联展示 + 键位）。
- cancel 确认对话框与 undo/restore（采用即时取消、无确认、无恢复）。
...
4. [x] Phase E 完成 `clientRequestId` 幂等链路、共同 prompt 投影、Web Queue UI（自适应 + 弹回编辑）、TUI 内联队列（`Alt+Up` + queued-edit 模式内 `Ctrl+D`）、`canSend`、edit lease 与 Playwright。
