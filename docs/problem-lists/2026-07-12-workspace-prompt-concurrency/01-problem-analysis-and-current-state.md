...
>
> **时间口径说明**：§1.1–1.13 描述后端 Phase A–D 实施前的历史问题基线。后端批次已完成，§1.1–1.13 中的 backend gap（如"events.ts 没有 prompt.submitted/updated"、"snapshot 没有 prompt submission collection"）已由 Phase A–D 关闭，本文保留为历史记录。§1.14 描述后端完成后的 Phase E 前端基线。
...
- 当前 submit/receipt/event 没有 `clientRequestId`，因此多 client 同 session 下无法判断某个 `prompt.submitted` 是否对应本浏览器的 in-flight attempt；按事件类型或文本猜测会误清 composer 或重复提交。
- `apps/ohbaby-web/src/ui/App.tsx#Composer` 的 draft 只是组件 `useState`，没有按 scope/session 持久化；刷新会丢未发送文本，snapshot/replay 也没有“不修改 composer”的显式边界。
...
- `packages/ohbaby-cli/src/tui/components/prompt/index.tsx` 已将普通 `↑/↓` 固定用于 history up/down，裸字符 `d` 也必须继续进入 composer；Queue 操作只能使用修饰键或明确的 queued-edit 模式，不能声称 `↑`/`d` 无冲突。

Phase E 的最小目标不是把后端全部状态字段搬上屏幕，而是建立一个简单投影：`queued → Queue 区`、queued cancel → 消失、其他已接受状态 → conversation、目标错误 → 对应 message；TUI 通过 composer 上方内联展示，`Alt+Up` 进入 queued-edit 模式，`Ctrl+D` 只在该模式取消当前 prompt，不使用 `/queue` 命令或独立管理面板。普通 `↑/↓` 的历史浏览语义必须保留。
