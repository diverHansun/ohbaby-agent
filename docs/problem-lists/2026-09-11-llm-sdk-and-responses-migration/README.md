...
- Improve 4 规划开启日期：2026-09-14；用户确认“用量与校准链路对齐”，正式草案见 [improve-4](./improve-4/README.md)。仅规划和审查，等待用户审核后另行实施。
...
- [`improve-4/`](./improve-4/)：用量与校准数据合同、最小参数命名和证据补齐；算法不变，规划待用户审核。
...
| improve-3     | 2026-09-13 | improve-2 §2.8主动切割的内部契约；用户授权提前规划                | 05 独立验收和完整补验通过（2026-09-14）；用户已授权本地合入。improve-4 规划已获授权，实施待审 |
| improve-4     | 2026-09-14 | improve-3 主动切出的用量/校准议题，承接其 05 与用户确认的收窄范围 | 00–04 规划草案；生产未实施，等待用户审核                                                      |
...
2026-09-14 当前依据为 [improve-4 正式规划草案](./improve-4/README.md)：只对齐供应商用量、单步估算和校准合同，保留算法、统计与占用数字行为。此前 [候选范围](./improve-3/next-stage-candidates.md) 仅作历史输入；不授权替换 wire 估算材料或原生续接。以下旧路线保留为历史候选，不覆盖本轮 00/02 边界。
