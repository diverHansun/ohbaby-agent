...
实现Model Context Protocol客户端，支持Stdio和HTTP/SSE两种传输方式，让ohbaby-agent能够连接和使用符合MCP协议的工具服务器。

### 2. 动态扩展工具能力

允许用户通过配置文件添加MCP服务器，动态扩展LLM可用的工具集，而无需修改ohbaby-agent代码。

### 3. 与tool-scheduler无缝集成

将MCP工具转换为ohbaby-agent的Tool接口，注册到ToolScheduler，使MCP工具与内置工具在调用层面保持一致。

### 4. 参与统一的并发调度

MCP 工具在适配时通过 `annotations.readOnlyHint` 推断 ToolCategory（`true` → `readonly`，其余 → `write`），注册到 ToolScheduler 后与内置工具走相同的并发控制路径。ohbaby-agent 负责工具调用的转发和并发协调，MCP 服务器负责工具的实际执行。
...
负责：
- 根据配置的 trust 字段映射通用 `Tool.requireExplicitApproval`
- 在所有模式（Ask/Plan/Agent）下开放MCP工具
- MCP 工具通过 `readOnlyHint` 分配 ToolCategory，注册后由 tool-scheduler 的权限状态与并发流程统一处理
...
### 6. 资源和 Prompt 的直接协议实现不属于 scheduler
...
| Policy | 间接关系 | MCP模块不直接调用Policy.check；适配后的Tool由tool-scheduler执行类别权限判断 |
| Permission | 无直接依赖 | `requireExplicitApproval` 可能触发 Permission.ask（由 tool-scheduler 处理） |
...
- 可以用一句话说明该模块的存在意义：mcp模块实现MCP协议客户端，让ohbaby-agent能够动态连接和使用第三方MCP工具服务器
