...
|------|------|---------|----------------|
| Phase 1 | 收紧 `core/agents` 运行契约并实现 `waitMode: "stream"` 分支 | 目标一、目标五前置 | 否（内部 port 收窄 + 已预留 stream 能力） |
| Phase 2 | `AgentService.startSession` 落地；composition / RunWorker 切到该入口；删除 `messages/buildPromptMessages` 过渡债务 | 目标一、目标二、目标六 | 否（新增方法 + adapter 内部重构；builder 随 lifecycle 接合同步退场） |
...
lifecycle improve-2 完成（推荐前置，不严格依赖）
        │
        ▼
Phase 1: AgentRunHandle + AgentRunResult union + runAgent stream 分支
        │
        ▼
Phase 2: AgentService.startSession + adapter 切换 + messages/builder 退场路径
        │
        ▼
...
## 二、Phase 1：收紧 `core/agents` 契约并实现 `runAgent` stream 分支

### 2.1 目标

完成 `core/agents.runAgent` 在 `waitMode: "stream"` 时的行为实现，同时收紧 Phase 1 会直接触碰到的运行契约。improve-1 期间 stream 分支仅做了类型预留与 `NotImplementedError`，本阶段填实；`AgentRunCoordinator.create` 与 `AgentRunResult` 一并改成更小、更明确的类型。
...
`permissionProfileId / multitaskStrategy / disconnectMode` 等 runtime 内部字段留在 RunManager 自己的记录类型里。`core/agents` 只依赖 handle。
...
调用方通过 `result.mode` 和 `result.success` 获得精确类型，不再靠 optional 字段恢复语义。
...
| `core/agents/types.ts` | `AgentRunRecord` 改为 `AgentRunHandle`；`AgentRunResult` 改为 discriminated union；新增 `AgentRunEventSource` |
| RunManager adapter / composition wiring | 把内部 RunManager 记录转换为 `AgentRunHandle`，不向 `core/agents` 泄漏 runtime 字段 |
...
export class AgentService {
  ...
  // improve-2 新增
  async startSession(params: StartSessionParams): Promise<AgentSessionStartResult> {
...
#### 3.2.4 `messages/buildPromptMessages` 过渡处理

Phase 2 同步接合 RunManager / RunWorker 与 `Lifecycle.runSession`，不再保留外部预组装 messages 的 core/agents 入口：

- `AgentRunCreateOptions.messages` 从 `core/agents` 启动契约中删除。
- `AgentRunInput.buildPromptMessages` 删除。
- `AgentPromptMessageBuilder` 类型删除，不保留 deprecated alias。
- RunManager 保留兼容旧 `messages` 路径供底层测试和非 agent 旧调用使用，但 agent 路径必须传 `directory / modelId`，由 `Lifecycle.runSession -> context.prepareTurn` 组装 turn messages。
- prompt/context 组装只保留在 `Lifecycle.runSession -> context.prepareTurn` 链路。

这一步体现 YAGNI 与 SRP：不长期保留两套 message 组装入口，agent 路径只暴露 session-run 所需的最小契约。
...
| `core/agents/types.ts` | 删除 agent 路径上的 `messages/buildPromptMessages/AgentPromptMessageBuilder`，改传 `directory / modelId` |
...
| `AgentPromptProvider`（agents 模块内部用） | `agents/types.ts` | 已在 improve-1 收尾重命名，避免与 `core/context.SystemPromptProvider` 冲突 | 命名清理 |
...
| Phase 2 | revert composition.ts / RunWorker session-run 切换 commit；如需回退，恢复旧 builder 只能作为临时兼容分支处理。 |
...
improve-1 只要求预留 `waitMode: "stream"` 能力与事件流概念。improve-2 Phase 1 会在填实实现时同步收紧类型：`AgentRunResult` 改为 discriminated union，`events` 只存在于 `mode: "stream"` 分支；`AgentRunCoordinator.create` 返回值也从完整 run record 收窄为 `AgentRunHandle`。
...
improve-2 切 primary 时，composition.ts 中 primary 专用 prompt builder 应整体删除；`AgentService.startSession` 只传 `sessionId / projectRoot / modelId / prompt` 等 session-run 输入，prompt/context 组装统一由 `Lifecycle.runSession -> context.prepareTurn` 完成。
