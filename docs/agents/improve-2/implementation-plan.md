...
| Phase 2 | `AgentService.startSession` 落地；composition / RunWorker 切到该入口；处理 `messages/buildPromptMessages` 过渡债务 | 目标一、目标二、目标六 | 否（新增方法 + adapter 内部重构；builder 退场视 lifecycle 接合点分批） |
...
Phase 2 需要显式处理 `AgentRunCreateOptions.messages` 与 `AgentPromptMessageBuilder` 的生命周期：

- Phase 2 完成前，`AgentRunCreateOptions.messages` 仍为必填，`runAgent` 继续调用 `buildPromptMessages`。这是为了兼容当前 RunManager create 契约，不代表长期方向。
- 当 lifecycle improve-2 已使 RunManager/RunWorker 可在 create 阶段不接收预组装 messages 时：
  - `AgentRunCreateOptions.messages` 改为 optional 或删除。
  - `AgentRunInput.buildPromptMessages` 改为 optional、标记 `@deprecated`，随后删除。
  - `AgentPromptMessageBuilder` 类型标记 `@deprecated`，文档注明它是过渡期产物。
  - prompt/context 组装只保留在 `Lifecycle.runSession -> context.prepareTurn` 链路。

这一步体现 YAGNI 与 SRP：不长期保留两套 message 组装入口，只在过渡窗口保留兼容字段。
...
| `core/agents/types.ts` | lifecycle 接合完成后，将 `messages/buildPromptMessages/AgentPromptMessageBuilder` 标记 deprecated 或移除 |
