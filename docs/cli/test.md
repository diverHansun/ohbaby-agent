...
| 场景 | 输入 | 预期结果 |
|------|------|----------|
| 无参数启动 | `iris-code` | `{ help: false, version: false, prompt: undefined }` |
| 帮助参数 | `iris-code -h` | `{ help: true }` |
| 版本参数 | `iris-code -v` | `{ version: true }` |
| prompt 参数 | `iris-code -p "测试"` | `{ prompt: "测试" }` |
| 未知参数 | `iris-code --unknown` | 抛出 CliArgumentError |
...
describe('parseArgs', () => {
  it('should parse help flag', () => {
    const args = parseArgs(['node', 'iris-code', '-h'])
    expect(args.help).toBe(true)
  })

  it('should parse prompt option', () => {
    const args = parseArgs(['node', 'iris-code', '-p', '测试'])
    expect(args.prompt).toBe('测试')
  })

  it('should throw on invalid option', () => {
    expect(() => parseArgs(['node', 'iris-code', '--invalid']))
...
describe('bootstrap', () => {
  it('should initialize in correct order', async () => {
    ...
    await bootstrap(['node', 'iris-code', '-p', 'test'])
  ...
  it('should call UI.render in interactive mode', async () => {
    ...
    await bootstrap(['node', 'iris-code'])
...
describe('exception handlers', () => {
  it('should exit with code 2 on argument error', async () => {
    ...
    try {
      await bootstrap(['node', 'iris-code', '--invalid'])
    } catch {}
