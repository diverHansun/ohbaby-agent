...
└──────────────────────────────────────────────────────────────────┘
          ...
          ▼               ▼               ▼
┌──────────────────────┐ ┌────────────────────────────┐ ┌────────────────────────────┐
│ Bootstrap（装配层）   │ │ app-events.ts（内部文件）   │ │ command-events.ts（内部文件）│
│                      │ │                            │ │                            │
│ 职责：               │ │ 职责：                     │ │ 职责：                     │
│ - 构建 RunDefaultsPolicy   │ - 订阅 app 级 Bus 事件      │ │ - 订阅 Commands.Event.*     │
│ - 按顺序创建子系统实例│ │ - 翻译为 StreamBridge app  │ │ - 订阅 Interaction.Event.*  │
│ - 注入依赖关系        │ │ - 返回 dispose() 供关闭    │ │ - 发布到 StreamBridge app   │
...
commandEvents     (← bus, streamBridge)
```
...
| **command-events.ts** | daemon 内部文件，Commands/Interaction 事件 → StreamBridge app scope 翻译 |
...
### 3. event adapter 作为内部文件而非独立模块

`daemon/app-events.ts` 和 `daemon/command-events.ts` 是 daemon 的内部实现文件，不作为独立 runtime 子模块。

**使用理由**：
- 当前翻译逻辑简单（Bus 事件 → bridge.publish），不需要独立测试矩阵。
- 分成两个文件可以避免 app-events.ts 过早膨胀：通用 app 事件在 `app-events.ts`，commands/interaction 事件在 `command-events.ts`。
- 升级条件已在 goals-duty.md 中明确：超过约 100 行、需要独立测试、或被 server/SDK 复用时，再提升为独立 runtime 子模块。
...
├── command-events.ts     # 内部文件：Commands/Interaction 事件 → StreamBridge app scope
...
| `command-events.ts` | 内部翻译 | 不对外导出；只被 bootstrap.ts 调用，专门处理 command.* 和 interaction.* 协议事件 |
...
- **内部实现**：bootstrap 装配顺序、app-events/command-events 翻译逻辑、pid 文件格式
