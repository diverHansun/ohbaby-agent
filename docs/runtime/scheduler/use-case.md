...
> **2026-07-13 修订（优先）**：创建/暂停/投递/合并等产品用例见 [`docs/loop/use-case.md`](../../loop/use-case.md)。本文件仅保留闹钟侧用例（注册、到期弹出、重算 nextFireTime、启动恢复）。  
> **2026-07-11 修订仍有效**：job 绑定 `scopeKey + sessionId`；Scheduler 只报到点；忙时 pending 不归本模块。下文经机器级 Heartbeat 决策的用例视为过时。
