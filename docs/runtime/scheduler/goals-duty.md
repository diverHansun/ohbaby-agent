...
- 只发布进程内触发信号；如需对外观测，由 `daemon/app-events.ts` 将 Bus 事件翻译为 StreamBridge 的 `app.scheduler.job-fired`
...
- **Reminder**（`kind = 'reminder'`）：对用户的"到点提醒"承诺，丢失代价高；触发后将 status 更新为 `completed` 或直接删除
...
scheduler 不直接调用 StreamBridge。对外的 `app.*` 事件由 daemon 层的 `daemon/app-events.ts` 内部 adapter 负责翻译。
...
| `runtime/stream-bridge` | 间接发布 | `daemon/app-events.ts` 将 `Scheduler.Event.JobFired` 翻译为 `app.scheduler.job-fired` 后通过 bridge 发布 |
