...
负责：
- 触发时调用注册的回调（或发出 `Scheduler.Event.JobFired` Bus 事件）
- 携带触发源信息（jobId / reminderId / followUpId）供 heartbeat 决策使用
- 只发布进程内触发信号；如需对外观测，由 daemon 持有的 app-event adapter 将 Bus 事件翻译为 StreamBridge 的 `app.scheduler.job-fired`

### 5. 任务持久化（可选，初期跳过）

负责（未来阶段）：
- 将 ScheduledJob / Reminder 持久化到 session storage，进程重启后恢复
- 初期阶段：仅内存维护，进程重启后任务丢失
...
scheduler 不直接调用 StreamBridge。对外的 `app.*` 事件由 daemon 层的 app-event adapter 或后续专门的 runtime event router 负责翻译。
...
| `runtime/stream-bridge` | 间接发布 | daemon 层 app-event adapter 将 `Scheduler.Event.JobFired` 翻译为 `app.scheduler.job-fired` 后通过 bridge 发布 |
| `services/session` | 未来依赖 | 任务持久化阶段将读写 session storage |
