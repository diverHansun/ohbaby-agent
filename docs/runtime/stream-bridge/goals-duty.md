...
### 4. 区分 run scope 与 app scope

`run.*` 事件按 runId 路由，只有订阅了对应 runId 的客户端才会收到。app scope 是进程级别的全局事件流，承载 runtime、permission、command、interaction、catalog 等 SDK 协议事件。bridge 只维护 `app` 与 `run/<runId>` 两类 scope，不为 commands 新增独立 scope。
...
负责：
- `publish('app', eventName, payload)` 广播给所有活跃 app 订阅者
- app scope 可以承载 `runtime.updated`、`permission.requested`、`command.started`、`interaction.requested` 等协议事件名
...
bridge 是被动的发布/订阅中介。决定"哪些 Bus 事件需要对外发布"是 runtime 事件翻译层的职责：`runtime/run-manager/worker` 负责 run scope，`daemon/app-events.ts` 负责通用 app scope，`daemon/command-events.ts` 负责 command/interaction 协议事件。bridge 只做传输。
...
| 模块 | 关系 | 说明 |
|------|------|------|
| `runtime/run-manager` | 被调用 | worker 调用 bridge.publish() 发布翻译后的 run scope 事件 |
...
| `bus` | 无直接依赖 | bridge 不订阅 bus，run scope 翻译由 run-manager/worker 承担，app scope 翻译由 daemon 内部 event adapters 承担 |
...
// 正确：run scope 逻辑在 runtime/run-manager/worker 中，
// app scope 逻辑在 daemon/app-events.ts / command-events.ts 中
```
