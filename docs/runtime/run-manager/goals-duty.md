...
`core/lifecycle` 负责 agent loop 的执行逻辑，但它只是一个可被调用的函数，不知道"这次调用是谁发起的、有没有在跑、能不能取消"。run-manager 的存在是为了给每一次 lifecycle.run 赋予身份（runId）、归属（sessionId）、来源（triggerSource）和状态（status），使其成为可查询、可取消、可追溯的运行单元。
...
### 4. 通过注入的 RunDefaultsPolicy 实现后台运行的安全默认值

后台触发的 run（scheduler / channel / heartbeat）没有 UI 可弹窗，必须在创建时就绑定权限画像和多任务策略。run-manager 消费装配层注入的 RunDefaultsPolicy，为每种触发源应用安全默认值，但不拥有这些默认策略本身，防止策略定义与执行机制混在一起。
...
负责：
- 创建 RunRecord（分配 runId，记录 triggerSource / sessionId / permissionProfile / multitaskStrategy / disconnectMode）
...
负责：
- `create()` 进入 per-session critical section，检查**内存索引**（`sessionId → active RunRecord[]`）是否已有 pending / running run；**不查询 run-ledger DB**（DB 不是热路径调度的仲裁器）
- 根据 `multitaskStrategy` 执行对应策略：
  - `reject`：直接返回错误，不创建新 run
  - `queue`：创建 run 但进入 pending 状态，等待前一个完成
  - `interrupt-current`：取消正在运行的 run，再启动新的
- **写入顺序**（保证账本先行）：
  1. 内存并发检测通过后，调用 `runLedger.createPending(runId, ...)` 写账本
  2. 账本写入成功后，将 RunRecord 放入内存索引
  3. 账本写入失败则 Run 不创建，内存索引不更新
- **worker 运行期**：worker start / end 时同步更新内存索引**和** run-ledger，两者都是写目标；但读取仲裁时只信任内存索引
- **ledger 写失败兜底**：若 worker 已完成但 ledger end 写失败，内存状态仍推进，记录错误并异步重试 ledger 更新；ledger 写失败不阻塞 worker 正常结束

### 3. 应用注入的 RunDefaultsPolicy

负责：
- 接收由装配层（daemon/bootstrap）注入的 `RunDefaultsPolicy`：`Record<TriggerSource, { permissionProfileId, multitaskStrategy, disconnectMode }>`
- 在 `create()` 时用 policy 补齐未显式声明的运行参数（merge defaults + explicit overrides）
- 保证最终生成的 RunRecord 完整记录所有实际生效的参数值，包括覆盖来源，便于审计
- 不拥有 policy 默认值；如调用方未注入 policy，run-manager 拒绝启动并抛出配置错误
...
| `runtime/permission-profiles` | 依赖 | 创建 run 时调用 getProfile(id) 解析 permissionProfileId 对应的画像实例；id 来自注入的 RunDefaultsPolicy |
...
| `runtime/run-ledger` | 依赖（写 + 恢复） | init() 时调用 markInterrupted()；create/start/end 时写账本；运行期并发仲裁只读内存索引，不把 run-ledger 当调度锁或队列 |
| `daemon/bootstrap` | 被注入 | bootstrap 构建 RunDefaultsPolicy 并在初始化时注入 run-manager |
| `bus` | 订阅 | worker 订阅 Bus 事件进行翻译 |
| `docs/ohbaby-sdk` / `ohbaby-sdk` | 类型依赖 | TriggerSource、PermissionProfileId、MultitaskStrategy、DisconnectMode 等窄类型定义在 sdk |
...
if (triggerSource === 'channel' && operation === 'write') {
