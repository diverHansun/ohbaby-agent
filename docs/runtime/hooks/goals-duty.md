...
- 定义以下 hook point：
  ...
  - `pre-tool`：tool-scheduler 调用工具前（可选实现）
  - `post-tool`：tool-scheduler 工具返回后（可选实现）
  - `on-wake`：heartbeat 决定创建 Run 时（唤醒前的准备）
- HookPoint 枚举先定义在 `runtime/hooks` 内部，不进入 `ohbaby-sdk`
- 只有当 hooks 作为插件系统的公开扩展点时，才考虑迁入 plugin SDK 或 ohbaby-sdk
...
- 定义传入 hook 函数的 context 类型：
  - `pre-run` / `post-run`：`{ runId, sessionId, trigger, permissionProfile, runRecord }`
  - `pre-tool` / `post-tool`：`{ runId, toolName, toolInput, toolOutput? }`
...
await hookExecutor.execute('pre-run', { runId, sessionId, trigger, runRecord })
const result = await lifecycle.run(session, abortSignal)
await hookExecutor.execute('post-run', { runId, sessionId, trigger, runRecord })
```
...
- 可以用一句话说明该模块的存在意义：runtime/hooks 提供 Run 执行前后的钩子机制，使 memory 注入、session 摘要、token 记录等横切关注点以可测试的 hook 函数组合，而不是堆入 lifecycle
- 能清楚回答"这个模块不该做什么"：不做动态加载、不实现业务逻辑、不处理 UI 钩子、不支持用户自定义 shell hooks、不做并行执行
- 职责与其他模块无明显重叠：core/lifecycle（agent loop 引擎）、core/memory（记忆实现）、runtime/run-manager（Run 管理）边界清晰
