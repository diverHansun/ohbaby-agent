...
- goal `paused` / `blocked` 时，普通用户消息应让模型看见一个轻量 goal note，但不得自动恢复 goal。

本补丁覆盖后端与 CLI 适配边界。Web 端暂不改；CLI 顶部的 goal 状态提示后续单独设计。

## 一、对标结论

Codex 和 Kimi 的共同点是：goal 有一个很薄的公开状态快照，objective 作为不可信任务数据注入模型上下文，恢复/完成/阻塞都通过状态机而不是散落的 UI 旗标表达。
...
不新增 `PauseCause`、`resumeIntent`、`interruptedBy` 等持久字段。后续 CLI 状态条只消费已有 `GoalSnapshot.status`、`terminalReason`、进度和 budget 字段。
...
3. goal 状态转为 `paused`，`terminalReason` 使用简单人读字符串，例如 `interrupted`。
...
- `blocked`：轻提醒。说明存在一个阻塞 goal和 reason，当前不会被自动推进；用户可讨论阻塞原因，也可 `/goal resume` 重新尝试。
- 无 goal / `complete`：空。
...
- `terminalReason` 与 objective 一样是模型可见的不可信数据；若写入 light note，必须包裹/转义，不能裸插入指令文本。
...
3. 若状态是 `paused` 或 `blocked`，把 `renderGoalContextNote(snapshot)` 拼到发给模型的 prompt 前。
4. UI 侧 `message.appended` 仍展示用户原始 text。
5. goal continuation prompt 本身不附加 light note，避免 active 提醒和 paused/blocked 提醒混杂。
...
- `goals/injection.unit.test.ts`：paused light note、blocked light note、无 goal 空输出；objective 和 completion criterion 仍做 `<untrusted_*>` 包裹与转义；light note 不含 `GOAL_CONTINUATION_CORE`。
...
- `ui-inprocess.contract.test.ts`：paused / blocked goal 下普通 prompt 可见 light note，但不会自动触发 `resumeGoal` 或 GoalDriver。
