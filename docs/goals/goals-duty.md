...
- GoalDriver 为每个 goal-owned Run 明确携带该轮启动时的 `goalId`，供 adapter 冻结 Todo workload ownership；goals 不读取 Todo 内容
...
- 复杂 Goal 可由模型使用 Todo 表达里程碑并跨 continuation 维护；complete 前对账是 prompt 软契约，不是 GoalService 状态迁移的硬门禁
...
| todo-list / adapter scope lease | 平行模块 + adapter 集成 | GoalDriver 只提供 run-start goalId；adapter 将其映射为内部 `goal:<goalId>` Todo scope，GoalService 不读取或校验 Todo |
