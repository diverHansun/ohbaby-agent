...
长任务最大的风险是"跑飞"——在已完成或不可能的目标上空转。终止靠四条：**模型自审**（每轮自审后通过工具声明 complete / blocked，主）、**用户控制**（`/goal pause` / `cancel` / Esc，随时）、**opt-in 预算**（用户可选设 turn/token/time 上限，到顶自动 block，可恢复）与**不可配置安全阀**（未设预算时，续跑轮数达写死上限即 `markBlocked`，纯防无限循环）。预算是 opt-in 的——不设则无约束，设了则按维度独立判定。安全阀仅在用户未设 turn 预算时作为兜底。单轮内的 step-runaway 由运行时既有的 per-run step 上限（`DEFAULT_MAX_STEPS`）兜住。
...
- 定义生命周期状态机：`active`（唯一推进态）/ `paused`（用户或运行时停的，可恢复）/ `blocked`（系统停的，带 `terminalReason`，可恢复）/ `complete`（瞬态，宣告即清、从不落盘）
- 记录 `GoalActor`（`user | model | runtime | system`）与 `terminalReason`，用"谁停的 + 原因"折叠状态：不可能 = `blocked`+reason，运行时/模型报错 = `paused`+reason，取消 = 丢弃记录
- **不区分暂停缘由的枚举**：所有 `paused` 同等处理，恢复路径统一为 `/goal resume`。`terminalReason` 仅供 UI 展示，不驱动恢复逻辑分支
- 作为所有状态迁移的**唯一入口**（create / resume / pause / markBlocked / markComplete / cancel / replace / incrementTurn / recordTokenUsage），并在入口处校验 actor 边界与迁移合法性

### 2. 编排续跑循环（GoalDriver）

负责：
- 在 `runtime/run-manager` **之上**编排：为 goal 起一个续跑 Run，`waitForCompletion` 读取 `{status, terminalReason}`，据此决定 pause / block / 续投下一轮
- 每轮先递增 turn 计数，使"模型声明 complete 的那一轮"被计入统计
- 把各终止分支映射到状态迁移：run cancelled → `pause`，run failed → `pause`，模型已清记录或非 active → 结束循环，**预算到顶 → `markBlocked`**，**续跑轮数达安全阀（仅未设 turn 预算时）→ `markBlocked`**
- 每轮循环顶部检查预算与安全阀（借鉴 kimi 的跨轮守卫骨架：循环顶部判定→到顶 markBlocked）
- 只在 goal `active` 时推进；paused / blocked / complete 都不续投
...
负责：
- 提供 `CreateGoal` / `UpdateGoal`（active / blocked / complete）/ `GetGoal` / `SetGoalBudget` 工具定义与执行逻辑，供模型在自审后声明终止、读取当前状态或记录用户要求的预算
- 工具经 `sessionId` 定位并变更本 session 的 goal store（复用 tool-scheduler 现有的 `ToolExecutionContext`）
- `SetGoalBudget` 是 opt-in 的：仅当用户明确要求设预算时模型才调用；不设则无预算约束，靠安全阀兜底

### 4. 生成每轮续跑的提醒文本并写入 history

负责：
- 提供 GoalInjector：根据当前 goal 状态产出续跑提醒文本（纯函数、无副作用）。三档强度：`active`（全量提醒 + 进度 + 预算报告 + 自审指令）、`paused` / `blocked`（轻提醒，不自动干活，提示可 `/goal resume`）、无 goal（空）
- 以 `<untrusted_objective>` / `<untrusted_completion_criterion>` 包裹并做 HTML 转义
- **提醒文本作为续跑 Run 的 user 消息写入持久 history**（仿 kimi 的 append-only 方式），而非临时注入。旧提醒可被 compact 自然压缩，最新提醒始终在尾部。这保证：进程崩溃后 history 仍承载 goal 上下文；无需新建通用注入子系统

### 5. 提供生命周期命令的后端能力

负责：
- 为 `commands` 层暴露 goal 生命周期操作：创建（`/goal <objective>`）、查询（`/goal status`）、`pause` / `resume` / `cancel` / `replace <objective>`
- 命令层只做解析与转发，具体的状态迁移与合法性判断由 goals 承担

### 6. 定义持久化数据模型并委托存储

负责：
- 定义 goal 的持久化数据模型（事件/快照结构、turnsUsed、tokensUsed、wallClockMs、budgetLimits、terminalReason）与从记录重建的逻辑
- 把索引/元数据的落盘委托给 `services/database`（与 snapshot 模块同一姿态），不自建存储层
- 提供恢复归一化（`normalizeAfterReplay`）：进程重启 / `--resume` 后，把落盘的 active goal 降级为 paused

### 7. 暴露 goal 快照供上层展示

负责：
- 提供 goal 快照（objective、status、turnsUsed、tokensUsed、wallClockMs、budgetLimits、budgetReport、terminalReason 等机器可读结构），供 CLI `/goal status` 与后续 Web UI（button / 卡片）读取
- 只输出数据，不耦合具体渲染
...
每轮上下文的拼装、token 估算与压缩由 `core/context` / `core/system-prompt` 负责。goals 只**产出**续跑提醒文本并作为 user 消息写入 history，由现有的消息系统持久化。提醒在 history 中的位置、是否被 compact 压缩，由 context 模块按既有规则决定。**无需新建通用注入子系统**——goals 用现有的消息 append 机制即可。
...
goals 暴露快照与生命周期操作，具体的 CLI 文本渲染、Web button / 卡片由 UI 层实现。首版**不含** `/goal next` 排队与交互式管理面板——这些留待后续，但快照结构预留了它们所需的数据。
...
| `runtime/run-manager` | 依赖 | GoalDriver 调用 `create()` / `waitForCompletion()` 编排续跑 Run；用户插话用 `interrupt-current` 策略；run-manager 保持 turn-agnostic，不感知 goal |
...
while (true) {
  ...
  if (isBudgetExhausted(goal) || isSafetyCapReached(goal)) {
    await store.markBlocked({ reason: "budget or safety cap reached" })
    return
  }
  ...
  if (next === null || next.status !== "active") return // 模型已 UpdateGoal(complete/blocked)
}
```

正确：UpdateGoal 工具经 sessionId 变更 goal store
```typescript
// goals/tools/update-goal.ts（示意）
execute(params, ctx) {
...
```typescript
// goals/tools/set-goal-budget.ts（示意）
execute(params, ctx) {
  const store = goals.storeFor(ctx.sessionId)
