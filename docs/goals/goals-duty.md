...
长任务最大的风险是"跑飞"——在已完成或不可能的目标上空转。终止靠四条：**模型自审**（每轮自审后通过工具声明 complete / paused，主）、**用户控制**（`/goal pause` / `cancel` / Esc，随时）、**opt-in 预算**（用户可选设 turn/token/time 上限，到顶自动 pause，可恢复）与**不可配置安全阀**（未设预算时，续跑轮数达写死上限即 pause，纯防无限循环）。预算是 opt-in 的——不设则无约束，设了则按维度独立判定。安全阀仅在用户未设 turn 预算时作为兜底。单轮内的 step-runaway 由运行时既有的 per-run step 上限（`DEFAULT_MAX_STEPS`）兜住。
...
- 把各终止分支映射到状态迁移：run cancelled → `pause`，run failed → `pause`，模型已清记录或非 active → 结束循环，**预算到顶 → `pause`**，**续跑轮数达安全阀（仅未设 turn 预算时）→ `pause`**
