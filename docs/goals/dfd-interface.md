...
5. 依 `RunCompletion.status` 分支（把执行结果翻译成状态迁移）：
   ...
   - `cancelled` → pause（`terminalReason: "interrupted"`）→ 退出循环。
...
一条不变量：**只有 `active` 才推进**；循环退出即意味着 goal 进入 paused/blocked/complete 之一。
...
1. 任何迁移 → 发布 `goal.updated{snapshot, change}` 经 stream-bridge → CLI / Web 渲染（模块外）。
