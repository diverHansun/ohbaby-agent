...
- glob 与真动态路径的区分。
- sandbox 对外部路径、sensitive path 和 denylist 的消费。
- scheduler 的 external-first / sensitive-path permission 顺序。
...
- `cat ../outside/.env` 先触发 `external_directory`，批准后再触发 `sensitive_path`，最后再按 bash readonly 放行。
- `cat .env` 触发 `sensitive_path`，`.env.example` 不触发。
- `cat *.md` / `rm build/*.tmp` 不因 glob 直接抛错。
