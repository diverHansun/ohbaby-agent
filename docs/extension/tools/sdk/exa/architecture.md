...
Exa 工具模块是 iris-code 的扩展工具层，封装 exa-js SDK 提供 Web 搜索和内容获取能力。作为 network 类别工具，为 Agent 提供互联网信息检索功能。
...
**配置来源优先级**：
1. 项目级别：`{project}/.iris-code/tools/exa.yaml`
2. 用户级别：`~/.config/iris-code/tools/exa.yaml`
...
将 exa-js SDK 的 camelCase 接口适配为 iris-code 的 snake_case 参数风格。
