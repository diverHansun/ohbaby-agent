...
所以包的真实状态是：**已抽包，但 `server.ts` 仍是手写 Node `http` + 单一 jsonrpc 信封，Hono 转换 + web surface 那一刀还没落。** 本目录就是落这一刀的设计。
...
把唯一的 agent backend，经一个 **Hono app** 暴露给多前端（CLI-attach / web / 未来 app）：**默认 CLI 不经过它**（直连 backend，守 N2），`ohbaby serve` 才 `listen`，web/app 与测试 harness 经 `app.fetch` 注入式调用。两条消费路径在 `CoreApiHost` 契约层统一，杜绝漂移。
