...
> v0.1.6 的 web 前端（见 [`../../ohbaby-web/`](../../ohbaby-web/README.md)）同源继承伺服它的 daemon 的 workspace scope，自身不解析 scope。其设计对本节提出依赖 **S-D**：

- **scope = git-root**，per-scope `.ohbaby/` 启动锁：同一 scope 并发 `serve` 只产生一个 daemon。建议偷师 kimi `packages/server/src/lock.ts` 的锁语义——记 `pid+port+token+version+scope`，做 **pid 探活 + stale takeover + 版本握手**，直接根治"daemon 死了/版本不符 → `fetch failed`"。
- **v0.1.6 落 Option A（一 serve 一 scope）**，但状态文件字段与 `/v1` 寻址**预留 scope 维度**，便于将来升级到本文的 InstanceStore + `x-ohbaby-directory` 多实例（Option B / Δ7）代价最低。web 内"多项目界面切换"属 web 的 ND5，本期不做。
