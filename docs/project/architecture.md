...
全局数据目录（XDG 标准）：
├── Linux/macOS: ~/.local/share/ohbaby-agent/
├── Windows: %LOCALAPPDATA%/ohbaby-agent/
└── 用于存储：sessions、messages 等

项目级配置目录：
├── {projectRoot}/.ohbaby-agent/
└── 用于存储：项目级设置
...
const projectConfigPath = path.join(projectRoot, '.ohbaby-agent', 'settings.json')

// Memory 模块使用（OHBABY.md 在项目根目录，不在 .ohbaby-agent/ 内）
const projectRoot = (await Project.fromDirectory(cwd)).rootPath
const memoryFilePath = path.join(projectRoot, 'OHBABY.md')
```

**说明**：
- 项目配置（settings.json、agents/*.json）存放在 `.ohbaby-agent/` 目录
