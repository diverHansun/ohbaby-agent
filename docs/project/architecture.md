...
全局数据目录（XDG 标准）：
├── Linux/macOS: ~/.local/share/iris-code/
├── Windows: %LOCALAPPDATA%/iris-code/
└── 用于存储：sessions、messages 等

项目级配置目录：
├── {projectRoot}/.iris-code/
└── 用于存储：项目级设置
...
const projectConfigPath = path.join(projectRoot, '.iris-code', 'settings.json')

// Memory 模块使用（IRIS.md 在项目根目录，不在 .iris-code/ 内）
const projectRoot = (await Project.fromDirectory(cwd)).rootPath
const memoryFilePath = path.join(projectRoot, 'IRIS.md')
```

**说明**：
- 项目配置（settings.json、agents/*.json）存放在 `.iris-code/` 目录
- 项目记忆（IRIS.md）存放在项目根目录，与 `.gitignore` 同级
- 这样设计使 IRIS.md 更易被用户发现和编辑，也便于加入版本控制与团队共享
