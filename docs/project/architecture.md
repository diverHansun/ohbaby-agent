...
全局数据目录（XDG 标准）：
├── Linux/macOS: ~/.local/share/ohbaby-code/
├── Windows: %LOCALAPPDATA%/ohbaby-code/
└── 用于存储：sessions、messages 等

项目级配置目录：
├── {projectRoot}/.ohbaby-code/
└── 用于存储：项目级设置
...
const projectConfigPath = path.join(projectRoot, '.ohbaby-code', 'settings.json')

// Memory 模块使用（OHBABY.md 在项目根目录，不在 .ohbaby-code/ 内）
const projectRoot = (await Project.fromDirectory(cwd)).rootPath
const memoryFilePath = path.join(projectRoot, 'OHBABY.md')
```

**说明**：
- 项目配置（settings.json、agents/*.json）存放在 `.ohbaby-code/` 目录
