...
- 写入成功后将路径更新到 `snapshot_patch.artifact_path`
...
- 如果 DB 记录存在但 artifact 缺失，`readArtifact()` 返回 `ArtifactNotAvailableError`，由 snapshot 核心层决定是否重新生成或提示用户
...
2. 一个 Turn 对应一个 checkpoint，checkpoint 下可能有零个或多个 patch（多轮提交场景）
3. artifact 文件大小适合文件系统存储（通常是单次 Turn 的文件 diff，不是整个工作区镜像）
