...
config/mcp 是 ohbaby-agent 配置系统的一部分，专门负责MCP服务器配置的加载和验证。它遵循与config/llm相同的设计模式，保持配置加载逻辑的独立性和纯净性。
...
1. loadMcpConfig() 被调用
   ↓
2. 加载全局配置文件 (~/.ohbaby-agent/mcp/settings.json)
   ↓
3. 加载项目配置文件 ({project}/.ohbaby-agent/mcp/settings.json)
   ↓
4. 合并配置（项目覆盖全局）
   ↓
5. Zod Schema 验证
...
- 与ohbaby-agent其他配置保持一致（model.json）
- TOML需要额外依赖和解析逻辑
- 可在未来根据需求扩展TOML支持
...
```typescript
// 工作区A的项目配置
{project-a}/.ohbaby-agent/mcp/settings.json

// 工作区B的项目配置
{project-b}/.ohbaby-agent/mcp/settings.json
...
```typescript
// ohbaby-agent/src/config/index.ts
