...
Exa 配置加载器是 iris-code config 模块的一部分，负责加载和管理 Exa 工具的配置文件。遵循 XDG 标准配置方案。
...
| 用户级 (Linux/Mac) | `~/.config/iris-code/tools/exa.yaml` | `$XDG_CONFIG_HOME` |
| 用户级 (Windows) | `%APPDATA%/iris-code/tools/exa.yaml` | `%APPDATA%` |
...
function resolveProjectConfigPath(projectRoot: string): string {
  return path.join(projectRoot, '.iris-code', 'tools', 'exa.yaml')
}
```
...
│     ▼                                                                    │
│  2. 检查用户级配置是否存在                                                │
│     ~/.config/iris-code/tools/exa.yaml                                  │
│     │                                                                    │
│     ├── 存在 → 加载并合并                                                │
...
│     ▼                                                                    │
│  3. 检查项目级配置是否存在                                                │
│     {project}/.iris-code/tools/exa.yaml                                 │
│     │                                                                    │
│     ├── 存在 → 加载并合并（覆盖用户级）                                   │
