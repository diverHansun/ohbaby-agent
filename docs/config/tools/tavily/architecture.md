...
Tavily 配置加载器是 ohbaby-code config 模块的一部分，负责加载和管理 Tavily 工具的配置文件。遵循 XDG 标准配置方案。
...
| 级别 | 路径 | 环境变量 |
|------|------|----------|
| 项目级 | `{project}/.ohbaby-code/tools/tavily.yaml` | - |
| 用户级 (Linux/Mac) | `~/.config/ohbaby-code/tools/tavily.yaml` | `$XDG_CONFIG_HOME` |
| 用户级 (Windows) | `%APPDATA%/ohbaby-code/tools/tavily.yaml` | `%APPDATA%` |
...
function resolveUserConfigDir(): string {
  ...
  if (process.platform === 'win32') {
    return path.join(process.env.APPDATA || '', 'ohbaby-code', 'tools')
  }

  // Linux/Mac (XDG)
  const xdgConfigHome = process.env.XDG_CONFIG_HOME ||
    path.join(os.homedir(), '.config')
  return path.join(xdgConfigHome, 'ohbaby-code', 'tools')
}

function resolveProjectConfigPath(projectRoot: string): string {
  return path.join(projectRoot, '.ohbaby-code', 'tools', 'tavily.yaml')
}
```
...
│     ▼                                                                    │
│  2. 检查用户级配置是否存在                                                │
│     ~/.config/ohbaby-code/tools/tavily.yaml                                │
│     │                                                                    │
│     ├── 存在 --> 加载并合并                                              │
...
│     ▼                                                                    │
│  3. 检查项目级配置是否存在                                                │
│     {project}/.ohbaby-code/tools/tavily.yaml                               │
│     │                                                                    │
│     ├── 存在 --> 加载并合并（覆盖用户级）                                 │
