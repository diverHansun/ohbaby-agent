...
`external_directory` 这个枚举值早已存在，但全仓库**没有任何 `ask({ type: "external_directory", ... })` 调用**。
原因正是 sandbox 这一层不报"哪些路径在外部"这个事实，permission 想问也无从问起。
...
> 哪些路径在 workspace 内、哪些解析到 workspace 外、哪些撞了敏感路径黑名单。
...
### 4. 边界检查 + 黑名单（`src/sandbox/boundary.ts` + `src/sandbox/denylist.ts`）

- **做什么**：判断一个绝对路径是否在 workspace 之内、是否撞中敏感路径黑名单（`~/.ssh`、`~/.aws`、`~/.gnupg`、`.env`、`*.pem`、`*.key`）
- **怎么用**：`boundary.classify(path, workdir) → "inside" | "outside" | { denylist: <reason> }`
- **外部路径不是错误**：进入 `externalPaths`，由 scheduler 先触发 `external_directory` ask
- **黑名单是 hard-deny**：不进 permission ask 流程，直接拒绝
...
3. **denylistHits 非空 ⇒ permission 必须拒绝**。这是 sandbox 对 permission 的强约束，
   permission 不应该把 denylist 命中物再拿去 ask 用户。代码上由 scheduler 在 ask 前早返实现。
4. **externalPaths 非空 ⇒ scheduler 必须先走 `external_directory` ask**。这是 external path
   priority：无论是绝对路径还是 `../` 解析出的外部路径，都先问外部目录权限；外部路径被批准后，
   再按原有 bash 权限规则判断是否还要问 `bash`。
5. **commands 数组对应多命令拆分**（`a; b && c` 拆成三个）。permission 按数组遍历问。
6. **PreflightResult 内的所有字段都是事实，不是建议**。permission 不需要"信任"sandbox 的判断，
   只需要消费数据。
...
## 与现有 PermissionType 的映射

improve-1 不引入新的 PermissionType，复用已有的两个：
...
- 黑名单命中即拒绝，但仅限**字面路径**——`rm -rf $(echo ~/.ssh)` 这种动态构造的路径，
