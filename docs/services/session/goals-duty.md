...
1. **依赖文件系统**：使用本地文件系统存储，不涉及数据库或远程存储
2. **单机环境**：当前版本不考虑多机同步或并发写入冲突
3. **依赖 Storage 模块**：底层文件读写通过 Storage 模块抽象
4. **依赖 Project 模块**：项目识别通过 Project 模块完成

### 假设

1. 同一 sessionId 不会被多个 lifecycle 实例同时使用
2. 文件系统可靠且写入操作原子性由操作系统保证
...
| Storage | `src/storage/` | 依赖 | `Storage.read/write` | 使用 Storage 进行底层文件读写 |
