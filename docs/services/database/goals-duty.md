...
在 daemon 模式下，只有 ohbaby-agent 后端进程直接写入 SQLite；CLI/TUI/Web/SDK 通过后端接口提交命令，不直接打开 DB 写入。在非 daemon 的本地 CLI 模式下，当前 CLI 进程本身就是后端入口。无论哪种运行方式，进程内部都共享一个 SQLite 连接实例，由 database 模块持有和管理。所有业务模块通过 database 模块获取 Drizzle 实例，不自行创建连接，避免连接生命周期和锁策略分散。
...
1. **单后端写入口**：同一工作区同一时刻只有一个 ohbaby-agent 后端入口直接写 DB。daemon 模式下该入口是 daemon；非 daemon CLI 模式下该入口是当前 CLI 进程。其他前端或外部接口通过后端接口间接操作。SQLite 层仍按一个写者 + 多个读者的规则序列化并发访问。
