...
### G1: 进程内统一连接管理，跨进程依赖事务仲裁

每个 ohbaby-agent backend 进程内部共享一个 SQLite 连接实例，由 database 模块持有和管理；业务模块不自行创建连接。全局 serve 内的多个 workspace runtime 复用该进程连接。默认 TUI 与显式 serve 可以作为两个合法进程同时打开同一 SQLite 文件，因此数据库正确性不能建立在“全机只有一个写进程”上：SQLite/WAL 串行化写事务，业务级同 session 互斥由 `run_ledger.claimPendingRun` 等显式事务不变量保证。
...
| `scheduler_job`（未来） | runtime/scheduler | 当前 migration 已删除；实现 session 级 `/loop` 时与 SchedulerStore 同批恢复 |
...
对写事务提供应用层 busy retry：
- 当 SQLite 返回 `SQLITE_BUSY` 时，最多重试 N 次
- 每次重试间隔加入随机 jitter，避免多个调用方或意外多进程同时重试形成竞争放大
- 重试耗尽后抛出明确的 `DatabaseBusyError`

在正常单后端入口模型下，业务写入主要在一个进程内串行进入 SQLite；Busy Retry 是防御性能力，用于处理外部读者、历史 CLI 进程、测试场景或平台文件锁造成的临时占用。

### D6: 数据库文件路径管理

确定 SQLite 文件的存储路径：
- 遵循 XDG 规范，与 `services/storage` 使用相同的基础目录
- 默认路径：`{baseDir}/ohbaby.db`（`baseDir` 为平台数据目录下的 `ohbaby/`）
...
1. **进程内单连接、跨进程可并存**：每个 backend 进程只通过 database 模块持有一个连接；默认 TUI 与 serve 可以并存。SQLite 串行化写事务，跨进程业务不变量必须由 transaction/claim 保证，不得恢复全局 backend lease 来伪造“全机单写者”。
...
| `runtime/scheduler`（未来） | 被依赖 | loop 批次恢复 `scheduler_job` 后，SchedulerStore 通过 database 模块读写 |
| `services/storage` | 无直接依赖 | 各自职责独立，共存互补 |
| `runtime/daemon` / CLI entry | 依赖 | 后端入口在启动时调用 `initDatabase()`，退出时调用 `closeDatabase()` |
...
-- 未来目标态：scheduler_job 表。
-- 当前 migration 已删除；仅在 session 级 /loop + SchedulerStore 同批实现时恢复。
-- FollowUp 不持久化；job 必须同时绑定 workspace scope 与 session。
