...
> `apps/ohbaby-web` 的模块级设计文档集。本包是 v0.1.6 引入的**轻量 web 端**——把全局 serve 中当前选中 workspace 的 agent backend，通过 daemon 的 `/v1` REST+SSE，呈现为浏览器里的交互式 coding-agent 界面。
>
...
- v0.1.6 不含多项目界面切换、完整 provider preset、interaction 请求。
- v0.1.7 全局单 serve 与 Web Phase 2 已完成：用户级 pid/state、请求级 workspace 路由、显式 directory header、项目 registry、服务端目录弹窗、rail、默认收起的 session sidebar，以及切换后的 client/snapshot/SSE 重绑均已落地。
...
| 多项目 scope | git-root；**全机一个 serve，按请求显式 directory 路由到 per-scope runtime** |
