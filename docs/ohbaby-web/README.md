...
- **最小 slash passthrough**：不做完整命令面板，只允许 web-safe allowlist（`/status`、`/help`、`/new`、`/mcps`、`/skills`），补齐 `commands` REST adapter、浏览器端解析/执行、命令结果/错误的轻量投影展示；需要 `interaction.requested` 的命令一律不进入 web passthrough。
- 完整 slash 命令面板、模型切换器、compact 交互面板、多项目界面切换、interaction 请求 → 后续版本。
