...
- **web-safe slash commands UI**：只允许 web-safe allowlist（`/status`、`/help`、`/new`、`/mcps`、`/skills`），补齐 `commands` REST adapter、浏览器端解析/执行、候选面板、Tab 补全、键盘选择，以及只读命令结果的结构化弹层展示；需要 `interaction.requested` 或未完成后端接线的命令一律不进入 web passthrough。
- `/connect`、`/connect-search`、`/compact` 的真实后端接线与交互式表单、模型切换器、多项目界面切换、interaction 请求 → 后续批次。
