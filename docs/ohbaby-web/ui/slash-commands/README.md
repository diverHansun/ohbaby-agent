...
> v0.1.6 的 slash UI 增强规格。设计参考来自 `ohbaby-web Slash.dc.html`，但命令集合以 daemon `/v1/commands?surface=tui` 返回并经 `ohbaby-sdk` web-safe helper 过滤后的 catalog 为唯一事实源。
...
- `/connect`、`/connect-search`、`/compact`。这些命令涉及模型配置、搜索 key、会话压缩 mutation，必须等后端真实接线和安全检查完成后再开放。
...
- UI 不硬编码可执行命令集合。
- `OhbabyWebClient.listCommands()` 返回已经过滤后的 `UiSlashCommandCatalog`。
- 浏览器执行前仍调用 `resolveSlashCommand(catalog, parseSlashCommandInput(text), { surface: "tui" })`。
- server 仍在 `POST /v1/commands` 再次用同一 SDK helper 校验 invocation。
...
- UI 不展示未接线命令，避免用户看到不可执行的配置/压缩承诺。
