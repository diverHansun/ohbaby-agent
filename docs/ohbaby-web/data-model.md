...
- **Entity（有身份、有生命周期）**：`StreamingMessage`（按 messageId 跟踪、随 delta 演进至定稿）、`PendingPermission`（按 requestId 跟踪、随用户应答或 resync 消失）。
...
- **更新**：均由 `eventReducer` 依据 SSE 事件推进；`lastAppliedSeqNum` 单调前进。
- **失效/销毁**：StreamingMessage 在 `message.updated` 定稿后并入消息序列；PendingPermission 在用户应答或 `resyncing` 重建后移除；整个 ViewState 在 `resyncing` 时被**整体丢弃重建**。
