...
- slash web-safe 闭环：`GET /v1/commands` / `POST /v1/commands`、browser resolve、候选面板/Tab 补全 helper、`command.*` 事件投影为 CommandNotice/结构化只读 modal。
...
| slash 候选与补全 | 只展示 web-safe catalog；`/connect`、`/connect-search`、`/compact` 不出现；Tab 补全当前选中命令 |
...
| 浏览器端 E2E（Playwright MCP） | 真实 `ohbaby serve --web-assets-dir` 页面：空态可见、键盘发送、mode/policy、slash palette、slash `/status` modal、基础响应式 | 页面崩溃/控制不可用即失败 |
...
- **e2e 仅轻量 smoke**：起真 daemon + 浏览器侧客户端走通"连接→发话→收流→审批/中断/web-safe slash palette + `/status` modal"。使用 Playwright MCP 做真实浏览器检查；重型像素级视觉回归暂缓。
