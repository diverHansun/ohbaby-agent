...
当用户选择 "always allow" 时，通过 Bus 通知 Policy 模块切换到 edit-automatically 模式。
...
- always：添加到批准列表，resolve Promise，通知 Policy 切换模式
...
- Permission.Event.SwitchModeRequested：请求 Policy 切换模式
...
模式（ask/plan/ask-before-edit/edit-automatically）的管理由 Policy 模块负责。permission 模块只通过 Bus 发送切换请求。
...
| 模块 | 关系 | 说明 |
|------|------|------|
| Policy | 被依赖/通知 | 工具调用 Policy 获取决策；permission 通过 Bus 通知 Policy 切换模式 |
