...
**ohbaby-agent 设计理念**（与 Claude Code/opencode 的差异）：
```
┌─────────────────────────────────────────────────────────────────┐
│                    ohbaby-agent Skill 设计                          │
├─────────────────────────────────────────────────────────────────┤
...
- **用户级**：`~/.config/ohbaby-agent/skill/`（Linux/macOS）或 `%APPDATA%/ohbaby-agent/skill/`（Windows）
- **项目级**：`{projectRoot}/.ohbaby-agent/skill/`
...
**注意**：ohbaby-agent 设计中，Skill 不作为工具暴露给 Agent。Agent 通过对话上下文接收 Skill 内容，而非通过工具调用。
...
ohbaby-agent 设计中，Skill 元数据默认不加载到系统提示。注入时机和方式由 Commands 模块控制。
...
| 平台 | 路径 | scope 值 |
|------|------|----------|
| Linux/macOS | `~/.config/ohbaby-agent/skill/` | `user` |
| Windows | `%APPDATA%/ohbaby-agent/skill/` | `user` |

### 项目级配置目录

| 路径 | scope 值 |
|------|----------|
| `{projectRoot}/.ohbaby-agent/skill/` | `project` |

### 目录结构示例

```
~/.config/ohbaby-agent/           # 用户级配置 (scope: 'user')
└── skill/
    ├── code-review/
    │   └── SKILL.md
    ├── commit-message/
...
{projectRoot}/.ohbaby-agent/      # 项目级配置 (scope: 'project')
└── skill/
...
| 模块 | 代码位置 | 关系 | 说明 |
|------|----------|------|------|
| **Commands** | `packages/ohbaby-agent/src/commands/` | **主要依赖方** | 注册 `/<skill-name>` 命令，调用 Skill 查询接口 |
...
- [x] ohbaby-agent 独特设计理念已明确说明（用户主导、按需加载）
