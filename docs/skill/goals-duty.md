...
**iris-code 设计理念**（与 Claude Code/opencode 的差异）：
```
┌─────────────────────────────────────────────────────────────────┐
│                    iris-code Skill 设计                          │
├─────────────────────────────────────────────────────────────────┤
...
- **用户级**：`~/.config/iris-code/skill/`（Linux/macOS）或 `%APPDATA%/iris-code/skill/`（Windows）
- **项目级**：`{projectRoot}/.iris-code/skill/`
...
**注意**：iris-code 设计中，Skill 不作为工具暴露给 Agent。Agent 通过对话上下文接收 Skill 内容，而非通过工具调用。
...
iris-code 设计中，Skill 元数据默认不加载到系统提示。注入时机和方式由 Commands 模块控制。
...
| 平台 | 路径 | scope 值 |
|------|------|----------|
| Linux/macOS | `~/.config/iris-code/skill/` | `user` |
| Windows | `%APPDATA%/iris-code/skill/` | `user` |

### 项目级配置目录

| 路径 | scope 值 |
|------|----------|
| `{projectRoot}/.iris-code/skill/` | `project` |

### 目录结构示例

```
~/.config/iris-code/           # 用户级配置 (scope: 'user')
└── skill/
    ├── code-review/
    │   └── SKILL.md
    ├── commit-message/
...
{projectRoot}/.iris-code/      # 项目级配置 (scope: 'project')
└── skill/
...
- [x] iris-code 独特设计理念已明确说明（用户主导、按需加载）
