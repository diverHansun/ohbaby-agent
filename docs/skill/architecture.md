...
```
1. 获取配置目录列表
   ├── Config.getUserConfigDir() → ~/.config/ohbaby-code/ (scope: 'user')
   └── Config.getProjectConfigDir() → .ohbaby-code/ (scope: 'project')
...
**注意**：ohbaby-code 设计中，默认不注册 SkillTool，因为 `disable-model-invocation` 默认为 `true`。
...
- [x] ohbaby-code 设计理念（用户主导、按需加载）已贯穿全文
