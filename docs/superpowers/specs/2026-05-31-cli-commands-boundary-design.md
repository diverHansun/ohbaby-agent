...
- 同一时刻只有一个 provider/baseUrl/apiKey/model 组合被应用。
- 允许通过 OpenAI-compatible 接口语义切换当前 provider、baseUrl、apiKey 和 model name。
- 单活动模型切换写回当前 `model.json` 顶层配置，并按 `apiKeyEnv` 写入或读取 `.env`/`process.env` 中的 API key。
- 切换调用路径是 SDK backend client 的 `setActiveModelConfig(input)`；`/models` 是 TUI 打开模型界面的入口。
- 如果当前配置只有一个模型，就只列出一个模型。
- 如果现有配置中已经包含多个 model profile，可以只读列出；本批次不做多模型 profile 的新增、修改、删除。
- 本批次不实现完整 TUI 配置表单；后续 TUI 在 `/models` 入口中采集 baseUrl、apiKey 和 model name，并调用 `setActiveModelConfig(input)`。
...
- 如果只有一个模型，不制造多模型交互。
- 暴露单活动模型切换所需的后端契约：`setActiveModelConfig(input)`。后续 TUI 表单负责采集 `provider`、`baseUrl`、`apiKey`、`apiKeyEnv` 和 `model`。
...
当前 `config/llm` 仍保持单模型读取和写入语义，不在 commands 分支中改成多 provider schema。
...
- 多个 provider/model profile 同时可管理的配置中心。

当前批次实现：

- 写回当前单活动 provider。
- 写回当前单活动 `apiConfig.baseUrl`。
- 写回当前单活动 `apiConfig.apiKeyEnv`。
- 在显式提供 API key 时写入 `~/.ohbaby-agent/.env` 并更新当前进程环境。
- 加载配置时优先读取 `process.env`，缺失时读取配置 API 传入的 `.env` 路径；启动入口仍可继续把项目和全局 `.env` 注入当前进程。
- 写回当前单活动 `defaultModel`。
- 切换成功后让后续请求使用新的 LLM 配置；正在运行的请求不被中断或迁移。
...
多 provider 配置中心、批量模型 CRUD 和正在运行请求的热迁移失败场景不在当前 commands 分支中处理。
...
- 多模型中心里选择模型后写回默认模型并热切换运行时 LLM client。
...
1. commands 分支：SDK resolver 权威化、catalog 优化、`/models` 单活动模型展示与切换契约、`/permission` 单入口。
