...
### N8: 不负责 TUI 交互和 CLI 命令解析

`/plugin install xxx` 的命令解析、TUI 确认弹窗、进度显示由 cli/commands 和 ohbaby-tui 模块负责。plugins 模块只提供 `install(source, scope)` 等无 UI 的 API。
...
| cli/commands | 被依赖 | CLI 调用 plugins 的 install/enable/disable/reload API |
| ohbaby-tui | 被依赖 | TUI 调用 plugins API，UI 层自行处理交互 |
