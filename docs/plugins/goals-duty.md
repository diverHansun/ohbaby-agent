...
`/plugin install xxx` 的 slash 词法解析由 `ohbaby-sdk` 负责，命令目录和执行入口由 backend `commands` 模块负责，TUI 确认弹窗和进度显示由 `ohbaby-tui` 负责。plugins 模块只提供 `install(source, scope)` 等无 UI 的 API。
...
| ohbaby-tui | 间接 | TUI 通过 SDK/commands 触发插件命令，UI 层自行处理交互 |
