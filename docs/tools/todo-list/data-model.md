...
- 0–10 项。
- 是某个 session/context/workload scope 的完整当前值。
- 写入时整体替换，不允许部分成功。
- 对外读取和事件传输均使用不可变副本。
...
该状态按 Todo-specific 复合 key `sessionId + contextScopeId? + workScopeId?` 保存。三者属于运行时上下文，不进入 TodoItem；主 context 的 `contextScopeId` 为空，ordinary 的 work scope 为空，Goal 的内部 work scope 为 `goal:<goalId>`。子 Agent `contextScopeId` 存在时不继承 parent work scope。

成功工具事务的 result metadata 可带 `internalWorkScopeId` 供恢复匹配。它是内部持久化细节，不属于 TodoItem、模型工具参数或公开 UI 类型；旧事务缺少该字段时只属于 ordinary。
...
ordinary run 只把 UI 投影 `visible` 设为 false，不删除 TodoList。active Goal 的相邻 continuation 共享同一 workload scope，边界不隐藏。

### 新 run/resume

ordinary 恢复列表后，存在 `pending` 或 `in_progress` 才重新设为可见；全 completed 历史保持隐藏。Goal resume 重新选择其 `goal:<goalId>` scope；active Goal 的非空列表即使全 completed 也保持可见，直到模型完成软对账并令 Goal 离开 active。
