...
**模块定位**：为 Agent 提供轻量、可恢复、按 session/context scope 隔离的执行清单，并通过正式 UI 投影在 Web/TUI 中显示当前进度。
...
每个主 session 或子 Agent session/context 独立保存当前完整列表。主 session 不读取或汇总子 Agent Todo。

### D4：从成功工具事务恢复

首次加载或 resume 时，从后向前查找最后一次已成功完成的 `todo_write`，忽略失败、拒绝、取消或不完整调用。成功的空数组也是有效恢复结果。

### D5：投影 snapshot、事件和显示生命周期

- 成功且内容发生变化的写入发布完整替换 `todo.updated`。
- `UiSnapshot` 可重建各 session 当前 Todo 和可见性。
- 当前 run 内非空列表可见；run 结束隐藏但不清空；新 run 仅在存在未完成项时重新显示。
...
Goal 管理长期目标、驱动状态和预算；Todo 仅描述当前执行步骤。
