...
if (!args.includes("--run")) {
    ...
    JSON.stringify(
      {
        profiles,
        command:
          "node scripts/run-real-connect-protocol.mjs --run --profile=<id>",
  ...
  const child = spawn(
    ...
    [
      ...
      "run",
      "--config",
      "tests/smoke/connect-protocol.vitest.config.ts",
    ],
    {
      env: {
        ...loadRootDotenv(),
        OHBABY_RUN_REAL_CONNECT_PROTOCOL: "1",
        OHBABY_REAL_CONNECT_PROFILE: selected,
      },
      stdio: "inherit",
    },
  );
  child.on("error", () => {
    console.error("Unable to start Stage A E2E process.");
    process.exitCode = 1;
  });
  child.on("exit", (code) => {
