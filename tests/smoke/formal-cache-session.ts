...
export interface FormalCleanupStep {
  code: string;
  run(): unknown | Promise<unknown>;
}

/** Each cleanup is attempted; only caller-chosen codes leave the process. */
export async function collectFormalCleanupErrors(
  steps: readonly FormalCleanupStep[],
): Promise<string[]> {
  const errors: string[] = [];
  for (const step of steps) {
    try {
      ...
      errors.push(step.code);
    }
  }
  return errors;
}

/** Preserve the primary test error while still attempting every cleanup and save. */
export async function finalizeReasoningEvidence(options: {
  primaryFailed: boolean;
  cleanup: readonly FormalCleanupStep[];
  cleanupErrors: string[];
  save(): Promise<void>;
}): Promise<void> {
  options.cleanupErrors.push(
    ...(await collectFormalCleanupErrors(options.cleanup)),
  );
  try {
    await options.save();
  } catch {
    options.cleanupErrors.push("EVIDENCE_WRITE_FAILED");
  }
  if (!options.primaryFailed && options.cleanupErrors.length)
    throw new Error("STAGE_C_CLEANUP_FAILED");
}

interface FormalSetupFailureEvidence {
  cleanupErrors: string[];
  diagnosticWorkspace?: string;
  wire: FormalCacheRequestEvidence[];
}
const setupFailures = new Map<unknown, FormalSetupFailureEvidence>();
export function getFormalSetupFailureEvidence(
  error: unknown,
): FormalSetupFailureEvidence | undefined {
  return setupFailures.get(error);
}
  ...
  try {
    await Promise.race([
      ...
      new Promise<never>((_, reject) => {
        timer = setTimeout(
          () => reject(new Error("OBSERVER_DRAIN_TIMEOUT")),
          5000,
        );
      }),
    ]);
...
export async function createFormalCacheSession(
  ...
  let setupObserver: ReturnType<typeof installFormalCacheObserver> | undefined;
  let setupBackend:
  ...
  try {
    ...
    setupRoot = root;
    if (options.emptyConfig) os.homedir = () => root;
    ...
    async function checkpoint(label: string): Promise<FormalCacheCheckpoint> {
      ...
      const contextEvidence = db
        ...
        .get(cache.sessionId);
      if (!contextEvidence)
        throw new Error("Missing persisted context evidence");
    ...
    return {
      ...
      async save(path: string): ReturnType<FormalCacheSession["save"]> {
        if (providerRequests.some((row) => !row.settled))
          throw new Error(
            "Provider operation still active; evidence not ready",
          );
        await observer.drain();
        if (providerRequests.some((row) => !row.settled))
          throw new Error(
      ...
      async close(): Promise<void> {
        cleanupErrors.push(
          ...(await collectFormalCleanupErrors([
            { code: "SESSION_UNSUBSCRIBE_FAILED", run: () => unsubscribe() },
            { code: "BACKEND_DISPOSE_FAILED", run: () => backend.dispose() },
            {
              code: "OBSERVER_DRAIN_FAILED",
              run: () => drainWithDeadline(observer),
            },
            { code: "FETCH_RESTORE_FAILED", run: () => observer.restore() },
            {
              code: "DATABASE_CLOSE_FAILED",
              run: () => closePersistentUiBackendDatabase(),
            },
            { code: "ENVIRONMENT_RESTORE_FAILED", run: restoreEnvironment },
          ])),
        );
        if (cleanupErrors.length)
          throw new Error("FORMAL_SESSION_CLEANUP_FAILED");
      },
      async reopen(): Promise<void> {
        ...
        if (
          sessionId &&
          !reopened.sessions.some((item) => item.id === sessionId)
        )
    ...
    const cleanupErrors = await collectFormalCleanupErrors([
      { code: "SETUP_UNSUBSCRIBE_FAILED", run: () => setupUnsubscribe?.() },
      { code: "SETUP_DISPOSE_FAILED", run: () => setupBackend?.dispose() },
      {
        code: "SETUP_DRAIN_FAILED",
        run: () =>
          setupObserver ? drainWithDeadline(setupObserver) : undefined,
      },
      {
        code: "SETUP_FETCH_RESTORE_FAILED",
        run: () => setupObserver?.restore(),
      },
      {
        code: "SETUP_DATABASE_CLOSE_FAILED",
        run: () => closePersistentUiBackendDatabase(),
      },
      { code: "SETUP_ENVIRONMENT_RESTORE_FAILED", run: restoreEnvironment },
    ]);
    setupFailures.set(error, {
      cleanupErrors,
      diagnosticWorkspace: setupRoot,
      wire: setupObserver?.records ?? [],
    });
    throw error;
  }
}
